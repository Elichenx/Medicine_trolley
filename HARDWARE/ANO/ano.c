...
void Ano_Anl(uint8_t *Data_Pack)          /* 数据解析 */
{
    ...
    if(Data_Pack[2] == 0xE1) /* 参数读取ID 0XE1 */
    {
        ...
        {            
            case 1: 
                Ano_SentPar(id,pidMotor1Speed.err_sum);        /* 速度 */
            break;
            
            case 2:
                Ano_SentPar(id,pidMotor1Speed.actual_val);     /* 圈数放大10倍发送 */
            break;
    ...
    if(Data_Pack[2] == 0xE2) /* 参数写入ID 0XE2，接收到的是整数*/
    {
        ...
        switch(id)
        {
            case 1:
                kp = *(int *)(&Data_Pack[6]);
                pidMotor1Speed.Kp = kp;
            break;
            
            case 2:
                ki = *(int *)(&Data_Pack[6]);
					 pidMotor1Speed.Ki=ki;
            break;                
        }
    }
